 ...
 */
static int
casecmp(register const char *s1,
...
/* FFI interface */
int
resolve(const char *domain, void (*a_callback)(struct dns_cb_data *))
{
  struct dns                *dns;
  fd_set                    set;
  ...
  dns_queue(dns, &domain, domain, DNS_A_RECORD, a_callback); 
...
int
callback_data_error_code(struct dns_cb_data *cbd)
{
  return cbd->error;
}

const char *
callback_data_name(struct dns_cb_data *cbd)
{ 
  const char *a_name = cbd->name;
  return a_name;
}

char *
callback_data_address(struct dns_cb_data *cbd, char *return_addr)
{
  sprintf(return_addr,
          "%u.%u.%u.%u",
